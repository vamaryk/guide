...
В исходном запросе одновременно указаны `Next.js` и `Vite`. Это два разных сборочных контура, их не стоит совмещать в одном простом MVP. Для текущего лендинга выбран `Vite`, потому что он напрямую соответствует связке `React + TS + Tailwind` и лучше подходит для быстрого одностраничного проекта.
